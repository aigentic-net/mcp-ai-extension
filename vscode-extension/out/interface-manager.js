...
class InterfaceManager {
    ...
    getInterfaceMode() {
        ...
        return config.get('interfaceMode', 'cursor-integration');
    }
    /**
     * Launch the standalone UI
     */
    async launchStandaloneUI() {
        const config = vscode.workspace.getConfiguration('aiextension');
        const standaloneUIPath = config.get('standaloneUIPath');
        if (!standaloneUIPath) {
            const result = await vscode.window.showErrorMessage('Standalone UI path not configured. Please set aiextension.standaloneUIPath in settings.', 'Open Settings');
        ...
        try {
            ...
            // Launch the standalone UI
            this.standaloneProcess = (0, child_process_1.spawn)('python', [standaloneUIPath], {
                detached: true,
                stdio: 'ignore'
            });
            ...
            vscode.window.showErrorMessage(`Failed to launch standalone UI: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
     * Inject content into Cursor chat
     */
    async injectIntoCursorChat(content, attachments) {
        try {
            ...
            if (activeEditor) {
                ...
                vscode.window.showInformationMessage('AI extension query copied to clipboard and shown in output. Paste into Cursor chat.', 'Open Output').then(selection => {
            ...
            vscode.window.showErrorMessage(`Failed to inject into Cursor chat: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
     ...
     */
    async handleInterfaceAction(content, attachments) {
     ...
     */
    async showInterfaceModeSelector() {
        ...
        if (selected) {
            ...
            vscode.window.showInformationMessage(`Interface mode changed to: ${selected.mode === 'standalone' ? 'Standalone UI' : 'Cursor Integration'}`);
        }
    }
    /**
     * Cleanup resources
